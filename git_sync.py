"""
Guarda archivos (modelos reentrenados, datos, cache, logs) de vuelta al
repo de GitHub del proyecto, para que sobrevivan un reinicio del
...
reentrenado, training_data_nba.csv actualizado, cache/game_summaries.json,
predictions_log_nba.csv) que no se haya subido a GitHub se pierde.
...
def commit_and_push(paths, message, secrets, cwd, max_retries=3):
    ...
    commit = _run(
        ["git", "-c", "user.email=nba-apuestas-app@local",
         "-c", "user.name=NBA Apuestas App", "commit", "-m", message],
        cwd,
    )
    ...
    for attempt in range(max_retries):
        ...
        # fallar directo con el error crudo de git.
